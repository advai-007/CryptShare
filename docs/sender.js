...
function sendFile(file) {
    const CHUNK_SIZE = 256 * 1024; // 256 KB (Increased from 64KB for speed)
    const progressBar = document.getElementById("progress-bar");
    const progressText = document.getElementById("progress-text");
    const speedText = document.getElementById("speed-text");
    const progressState = document.getElementById("state-progress");
    ...
    // avoid buffer overflow - Keep at 1MB to keep pipe full but not overflow
    ...
    function readNextChunk() {
        // Keep reading until buffer is full or file is done
        if (channel.bufferedAmount > channel.bufferedAmountLowThreshold) return;

        // If reader is busy, it will be called again by onload -> readNextChunk loop
